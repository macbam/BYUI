...
const char* theTestlistName = NULL;
bool passedFlag = true;

extern int asserts_existing;

void startTestlist(const char* aTestlistName){
    if(theTestlistName != NULL){
        if (passedFlag) {
            std::cout<<"Passed"<<std::endl;
        }
    }
    passedFlag = true;
    std::cout<<"Testing "<<aTestlistName<<"...";
    theTestlistName = aTestlistName;
}
